...
class MSHFunctionFixer:
    ...
    def normalize_function_signature(self, line: str) -> str:
        """规范化函数签名，确保参数格式正确"""
        # 处理各种参数格式，统一转换为标准格式
        # 1. 处理char *argv[]到char **argv的转换
        line = re.sub(r'(int\s+argc\s*,\s*)char\s*\*\s*argv\s*\[\]', r'\1char **argv', line)
        # 2. 处理char** argv到char **argv的转换
        line = re.sub(r'(int\s+argc\s*,\s*)char\s*\*\s*\*\s*argv', r'\1char **argv', line)
        # 3. 处理char**argv格式
        line = re.sub(r'(int\s+argc\s*,\s*)char\*\*argv', r'\1char **argv', line)
        # 4. 处理char* argv[]格式
        line = re.sub(r'(int\s+argc\s*,\s*)char\*\s*argv\s*\[\]', r'\1char **argv', line)
        # 5. 处理const char *argv[]格式
        line = re.sub(r'(int\s+argc\s*,\s*)const\s+char\s*\*\s*argv\s*\[\]', r'\1char **argv', line)
        # 6. 处理空参数或void参数
        line = re.sub(r'\(\s*void\s*\)', '(int argc, char **argv)', line)
        line = re.sub(r'\(\s*\)', '(int argc, char **argv)', line)
        
        # 确保参数格式正确
        line = re.sub(r'\(\s*int\s+argc\s*,\s*char\s*\*\*\s*argv\s*\)', '(int argc, char **argv)', line)
        
        return line

    def fix_function_signature(self, file_path: str, line_number: int, current_params: str, return_type: str) -> bool:
        """修复函数签名 - 改进版本，更精确地处理各种参数格式"""
        ...
        # 1. 修复参数格式 - 处理所有可能的参数格式
        if current_params != "int argc, char **argv":
            # 使用规范化函数处理参数格式
            new_line = self.normalize_function_signature(original_line)
        ...
        if return_type != "int":
            # 将非int返回类型改为int
            if 'static' in original_line:
                lines[line_idx] = re.sub(r'\b(void|long|rt_err_t|size_t|rt_size_t|rt_uint32_t|rt_uint32_t|rt_uint64_t|uint32_t|uint64_t)\b', 'int', lines[line_idx])
            else:
                lines[line_idx] = re.sub(r'\b(void|long|rt_err_t|size_t|rt_size_t|rt_uint32_t|rt_uint32_t|rt_uint64_t|uint32_t|uint64_t)\b', 'static int', lines[line_idx])
            modified = True
        ...
        for result in self.report_data['results']:
            if not result['is_compliant'] and result['definition']:
                ...
                if any('函数签名不符合规范' in issue for issue in issues):
                    if self.fix_function_signature(file_path, line_number, parameters, return_type):
