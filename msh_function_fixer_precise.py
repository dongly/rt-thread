...
class MSHFunctionFixer:
    ...
    def fix_function_signature_precise(self, file_path: str, line_number: int, current_params: str, return_type: str) -> bool:
        """精确修复函数签名 - 改进版本，更准确地处理各种参数格式"""
        ...
        if current_params != "int argc, char **argv":
            ...
            if 'char *argv[]' in new_line:
                ...
                modified = True
            elif 'const char *argv[]' in new_line:
            ...
            if current_params == "void" or current_params == "":
                ...
                modified = True
            elif current_params == "int argc, char** argv":
                new_line = re.sub(r'char\*\*\s*argv', 'char **argv', new_line)
                modified = True
        ...
        for result in self.report_data['results']:
            if not result['is_compliant'] and result['definition']:
                ...
                if any('函数签名不符合规范' in issue for issue in issues):
                    if self.fix_function_signature_precise(file_path, line_number, parameters, return_type):
